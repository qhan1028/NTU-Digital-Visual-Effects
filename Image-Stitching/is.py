...
def parse_txt(path):
    with open(path, 'r') as f:
        ...
        for i in range(1, len(data)-1):
            if data[i-1] is '\n' and data[i+1] is '\n':
                focals.append(float(data[i]))
    return np.array(focals)


def read_images(dirname, fix_h=480):
...
def process_images(images, focals, use_cw=True, bins=8, mthres=0.8, rthres=0.1, ransac_n=4):
    if use_cw:
        print('\n[Cylinder Warping]')
        print('focal length:\n', focals)
        images = cylinder_warping(images, focals=focals)
    ...
    print('\n[Matching, Blending]')
    dxy_sum = np.array([0, 0])
    dx_sum, dy_sum = 0, 0
    result = np.copy(images[0])
    for i in range(len(images)-1):
        print(i)
        matches = find_matches(D[i], D[i+1], mthres)
        dxy = ransac(matches, D[i], D[i+1], n=ransac_n)
        
        if dxy[0] > 0:
            dxy_sum += dxy
            dwh = np.array([dxy[0], dxy_sum[1]])
            result = blend(result, images[i+1], dxy_sum, dwh)
        else:
            result = blend(result, images[i+1], dxy, dxy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', type=float, nargs='?', default=0.1, help='R threshold.')
    ...
    # Example Usage
    rgbs, focals = read_images(args['input_dir'])
    result = process_images(rgbs, focals=focals, use_cw=args['cw'], bins=args['b'], mthres=args['m'], rthres=args['r'])
